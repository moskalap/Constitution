...
public class ConstitutionBuilder {
    ...
    private int getParagraphNo(int i) {
        ...
        int no = Integer.parseInt(words.get(i).split("\\s+")[0].substring(0, words.get(i).split("\\s+")[0].length() - 1));
        deleteFirstWord(i);
        return no;
    }

    private void deleteFirstWord(int i) {
        String[] separateWords = words.get(i).split("\\s+");
        String res = "";
        for (int k = 1; k < separateWords.length - 1; k++)
            res += separateWords[k] + " ";
        words.set(i, res + separateWords[separateWords.length - 1]);
