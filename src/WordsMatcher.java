...
public class WordsMatcher {
    ...
    public static Patterns whatItIs(String line){
        ...
        matcher=gównoPattern.matcher(line);
        if(matcher.matches())
            return Patterns.Garbage;
        matcher=gównoPattern2.matcher(line);
            if(matcher.matches())
                return Patterns.Garbage;
